...
A literate `2b4m` transliteration of **Chapter 2 ("The Integers")** of Thomas W.
...
Fundamental Theorem of Arithmetic. Run `2b4m check aata/2.1-induction.md`.

The integers are the axiomatic theory `std/integer.b4m` (a successor *and* a
predecessor, bidirectional induction) with its ring algebra
(`std/integer-ring.b4m`), order/subtraction (`std/integer/order.b4m`),
divisibility and powers (`std/integer/divides.b4m`), and — for ℕ-indexed
statements — the nonnegative integers with upward-from-0 induction
(`std/integer-nonneg.b4m`).

The integers are the axiomatic theory `std/integer.b4m` — a successor *and* a
predecessor, with a bidirectional induction schema over all of ℤ — together with
its ring algebra (`std/integer-ring.b4m`) and order/subtraction
(`std/integer/order.b4m`). We open by naming the arithmetic vocabulary the whole
...
In `2b4m` this principle *is* an axiom of the theory: `std/integer.b4m` has a
bidirectional induction schema over all of ℤ, and `std/integer-nonneg.b4m`
  ...
  @generalize-x |
    fix x: Int {
      @conclusion-difference-splits-off-three |
        add(mul(sub(x, ONE), FOUR), THREE) = sub(mul(x, FOUR), ONE)
        [using simplify definitionOfSubtraction mulZeroRight mulSuccRight addZeroRight addZeroLeft addSuccRight addPrevRight addSuccLeft addPrevLeft negSucc negZero oneIsSuccZero succPrev prevSucc addIsAssociative]
    }
  @conclusion |
...
proof
      ...
      @given-inductive-hypothesis |
        assume does_divide(THREE, sub(pow(FOUR, k), ONE)) {
          ...
          @three-divides-difference-times-four |
            ...
            [using specialize dividesMul(THREE, sub(pow(FOUR, k), ONE), FOUR) inductive-hypothesis]
          @three-divides-three |
            does_divide(THREE, THREE)
            [using specialize dividesRefl(THREE)]
          @three-divides-sum |
            does_divide(THREE, add(mul(sub(pow(FOUR, k), ONE), FOUR), THREE))
            [using specialize dividesAdd(THREE, mul(sub(pow(FOUR, k), ONE), FOUR), THREE) three-divides-difference-times-four three-divides-three]
          @sum-splits-the-successor-difference |
            add(mul(sub(pow(FOUR, k), ONE), FOUR), THREE) = sub(mul(pow(FOUR, k), FOUR), ONE)
            [using specialize successorDifferenceIdentity(pow(FOUR, k))]
          ...
          @successor-power-unfolds |
            ...
            [using specialize powSucc(FOUR, k)]
  ...
  @conclusion |
    forall n: Int; is_nonneg(n) -> does_divide(THREE, sub(pow(FOUR, n), ONE))
    [using instantiation nonnegInduction((fun m: Int => does_divide(THREE, sub(pow(FOUR, m), ONE)))) base-case induction-step-for-all-k]
qed
```
...
*equivalent* to the first principle. In `2b4m` that equivalence is discharged: both
...
ambient sort `Int` — the order is `std/integer/order.b4m`'s $<$ and $\le$, and
`std/integer/wellordering.b4m` proves strong induction and well-ordering over the
