...
pub fn addTests(
    ...
    ctx.ok(&.{ "check", "tests/cases/axioms_report.bpa", "direct", "--axioms" },
        \\OK: 9 declarations, 1 theorems proven
        \\  — rests on 1 axiom(s):
        \\      pZero  (tests/cases/axioms_report.bpa:10)
        \\
    );
    ctx.ok(&.{ "check", "tests/cases/axioms_report.bpa", "viaLemma", "--axioms" },
        \\OK: 9 declarations, 1 theorems proven
        \\  — rests on 2 axiom(s):
        \\      pZero  (tests/cases/axioms_report.bpa:10)
        \\      pImpliesQ  (tests/cases/axioms_report.bpa:11)
        \\
    );
    ctx.fail(&.{ "check", "tests/cases/hole.bpa", "restsOnHole", "--axioms" },
        \\error: 1 hole(s) remain (default mode rejects holes; use --draft while filling them):
        \\  - zeroIsEven  (tests/cases/hole.bpa:10)  — rested on by: restsOnHole
        \\
    );
    ctx.ok(&.{ "check", "tests/cases/hole.bpa", "restsOnHole", "--axioms", "--draft" },
        \\OK: 6 declarations, 1 theorems proven
        \\  — rests on 1 axiom(s):
        \\      zeroIsEven  (tests/cases/hole.bpa:10)  — HOLE
        \\  — DRAFT — 1 hole(s) unfilled (aspirational; the result is conditional on them): zeroIsEven; re-run `bpa check` (no --draft) once filled.
        \\
    );
