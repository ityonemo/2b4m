...
- **`z6.bpa`** — a concrete ℤ_6 = the (now-in-std) ℤ_n template instantiated at n = 6, via a
  ...
  Strict-green.
- **`overload-multisource-blocker.bpa`** — the minimal probe that exposes the
...
- **`q-z6.bpa`** — the CONCRETE target: ℤ_6 built over the ℚ-cut ℤ
...
through — the ℚ-cut satisfies all the ℤ axioms (via rational.bpa's guarded ℤ-model), so
...
```
bpa check scratch-needswork/integer-mod-n.bpa      # green
bpa check scratch-needswork/z6.bpa                 # green (ℤ_6 instance)
bpa check scratch-needswork/overload-multisource-blocker.bpa   # the block:
                                                   # "all model mappings must come from one source theory"
```
