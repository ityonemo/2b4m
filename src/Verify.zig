...
//!
//! TRUST MODEL (`--fast`): a `using` step (accelerant / instantiation / model / import) may be
//! TRUSTED — its proof is NOT generated or kernel-checked; instead the word ADMITS the step (an
//! accelerant runs its own cheap acceptance; model/import/instantiation α-match the cited
//! statement against the claim), the step is disclosed as accelerated, and NOTHING is cached (a
//! later strict demand redoes the full check). `by` primitives are ALWAYS kernel-checked, in
//! every mode. `Verify.trusted` is a per-word set over the `using` words; a word is trusted iff
//! it is in the set. The CLI builds it: `--fast` (all), `--fast W…` (allowlist), `--fast --slow
...
pub const Word = enum {
    ...
    /// The three ENGINE words — the `engine` group word.
    pub fn engine() Set {
        var s = Set.initEmpty();
        s.insert(.instantiation);
        s.insert(.model);
        s.insert(.import);
        return s;
    }
...
test "group words expand" {
    ...
    try testing.expect(Word.all().contains(.instantiation));
}
